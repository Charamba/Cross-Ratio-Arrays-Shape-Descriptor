...
arq_temp_file_path = sys.argv[1] # 'arq_temp.txt'
arq_test_file_path = sys.argv[2] # 'arq_test.txt'
folder_template_descriptors = sys.argv[3] # 
folder_test_descriptors      = sys.argv[4] #
folder_result                = sys.argv[5] # 
...
def main_test(template_nFanBeam_input, test_nFanBeam_input, emitter_points_number_input):
    ...
    if(file_type == 1):
        ...
        with open(arq_temp_file_path, 'r') as arq_temp:
            tem_filename = arq_temp.read()
            tem_filename = tem_filename.split()
        with open(arq_test_file_path, 'r') as arq_test:
    ...
    for t in tst_filename:
        ...
        # 'resultados/logos_e10/testes'
        file_name = folder_test_descriptors + '/' + str(test_nFanBeam) + test_name + '.txt'
        try:
        ...
        for x in tem_filename:
            ...
            if True:#(len(ncurves_test) == len(ncurves_temp)): 
                 
                file_name = folder_template_descriptors +'/' + str(template_nFanBeam) + temp_name + '.txt'
                try:
                    with open(file_name, 'rb') as myFile:
                ...
                if compare:
                    if compareByRays:
                        ...
                        # 'resultados/logos_e10/r'
                        with open(folder_result + '/r' +' ' + str(emitter_points_number) + ' ' + str(test_nFanBeam) + '.txt' , 'a') as arq:
                            arq.write(result)
                        with open(folder_result + '/r' +' ' + str(emitter_points_number) + ' ' + str(test_nFanBeam) + '_pairs.txt' , 'a') as arq:
                            arq.write(r)
