...
def show_matrix(head, matrix, flag):
	...
	table = tabulate(matrix, headers=head, tablefmt = "latex") 
...
def hmography(filename, filename_test, filename_temp, n_raios): 
    homo = {}
    ...
    while(i < len(h)-2):
        ...
        #    continue
        print("")
        with open(filename_temp+'/'+n_raios+temp_name+'.txt', 'rb') as myFile:
            templateDescriptor = pickle.load(myFile)
        with open(filename_test+'/'+n_raios+test_name+'.txt', 'rb') as myFile:
        ...
        if(pairs == []):
            ...
            #dist1, dist2 = find_homography_distance.calc_hausdorff_distance(templateDescriptor.hullVertices, testDescriptor.hullVertices, pairs)
            dist = find_homography_distance.calc_homography_distance(np.array(src), np.array(dst))
            #print(temp_name, test_name, dist1, dist2)
    ...
    (head, matx) = make_matrix(homo, 1)
    print(show_matrix(head, matx, 1))
    get_matrix_parameters(a, b, c, d)

def main():
    #matrix_type = int(input('1. Best Mach\n2.Sair\n'))
    #Colocar o nome do arquivo de pairs, diretorio da pasta de dos descritores de teste, direotiro da pasta dos descritores templates e o numero de raios utilizados 
    #hmography('resultados_elevation50_caracteres/elevation50/r 150 150_pairs.txt','resultados_elevation50_caracteres/elevation50/testes150', 'resultados_elevation50_caracteres/elevation50/templates150', '150')          
    
    result_pairs_file_path = sys.argv[1]
    template_descriptors_file_path = sys.argv[2]
    test_descriptors_file_path = sys.argv[3]
    #hmography('resultados_elevation50_caracteres/elevation50/r 150 150_pairs.txt','resultados_elevation50_caracteres/elevation50/testes150', 'resultados_elevation50_caracteres/elevation50/templates150', '150')          
    print("nada")
    hmography(result_pairs_file_path, test_descriptors_file_path, template_descriptors_file_path, '150')
