...
def fanbeams_threshold_distance(matchingVerticesPairs, distanceValues, threshold_factors=[]):
	# print("## original fanbeams distances:")
	# for i in range(0, len(distanceValues)):
	# 	print(distanceValues[i])

	avg = np.average(distanceValues)
	median = np.median(distanceValues)
	
	#print("median = ", median)
	if not(threshold_factors):
		fanbeamDistance_threshold = float('Inf') #median*1.5#avg*1.125 #(min(distanceValues) + max(distanceValues))*0.5
	else:
		[a, b, c, d] = threshold_factors
		fanbeamDistance_threshold = (avg*a + median*b)*c + d

	#print("median = ", median)
	#print("threshold = ", fanbeamDistance_threshold)
	#fanbeamDistance_threshold = 12#min(distanceValues)*2.125  #8# n/2.0
	matching_pairs_and_distances = [(pair, dist) for (pair, dist) in zip(matchingVerticesPairs, distanceValues) if dist < fanbeamDistance_threshold]
	[new_matchingVerticesPairs, new_distanceValues] = zip(*matching_pairs_and_distances)
	new_matchingVerticesPairs = list(new_matchingVerticesPairs)
	new_distanceValues = list(new_distanceValues)
	
	distanceValuesAux = deepcopy(new_distanceValues)
	distanceValuesAux.sort(reverse=True)

	# print("## filtered fanbeams distances:")
	# for i in range(0, len(distanceValuesAux)):
	# 	print(distanceValuesAux[i])
	
	return (new_matchingVerticesPairs, new_distanceValues)
...
class MatchingProcessor:
	...
	def compareByPencils_complete(self, symetric_shape=False):
		...
		fanbeamDistance_threshold = (min(distanceValues) + max(distanceValues))*0.5 

		#fanbeamDistance_threshold = 12#min(distanceValues)*2.125  #8# n/2.0
		matching_pairs_and_distances = [(pair, dist) for (pair, dist) in zip(matchingVerticesPairs, distanceValues) if dist < fanbeamDistance_threshold]
		[matchingVerticesPairs, distanceValues] = zip(*matching_pairs_and_distances)
		matchingVerticesPairs = list(matchingVerticesPairs)
		distanceValues = list(distanceValues)
		
		#print("distances: ", distanceValues)
		distanceValuesAux = deepcopy(distanceValues)
		distanceValuesAux.sort(reverse=True)
		...
		if len(distanceValues) != 0:
			...
			#print('Matching vertices Pairs: ', len(matchingVerticesPairs))
		...
		return (matching_result, new_pCoef, mTemplateRays, mTestRays, matchingVerticesPairsPoints, Distance, sigma, a, b)
